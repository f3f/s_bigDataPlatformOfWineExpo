...
barLine.prototype = {
    init: function (dom, data, xAxis) {
        ...
        me._rMax = Math.max.apply(this, me._data[1]);
        me._rMin = Math.min.apply(this, me._data[1])
    },
    show: function () {
        var me = this;
        var lastArr=[],newArr=[];
        var arr=me._data[0];
        var oldLength=me._data[0].length;     //原始数组的长度
        lastArr=me._data[0].pop();    //数组最后一个数据

        me._data[0].push('');     //补齐最后一个空数据的值

        for(var i=0;i<oldLength-1;i++){
            newArr.push('');
        }
        newArr.push(lastArr);     //前部分空值加最后数值
                    ...
                    itemStyle: {
                        normal: {
                            color: new echarts.graphic.LinearGradient(
                                0, 0, 0, 1,
                                [
                                    {offset: 0, color: '#7847f6'},
                                    {offset: 0.5, color: '#7847f6'},
                                    {offset: 1, color: '#7847f6'}
                                ]
                            )
                        },
                        emphasis: {
